...
function SoundManager(server) {
    var myPosition = [0, 0, 0];
    var thisRef = this;

    function myPlaySound(sound, loop, position) {
        var soundToPlay = null;
    ...
    server.addServerReceiveCallback(function(id, message){
        if(message.type === "sound") {
            ...
            for(i = 0; i < playersList.length; i++) {
                if(playersList[i] !== id) {
                    server.sendToClient(playersList[i], message);
                }
            }
        }
    });
    ...
    server.addClientReceiveCallback(function(message) {
        if(message.type === "sound") {
            myPlaySound(message.sound, message.loop, message.position);
        }
    });

    this.playSound = function(sound, position, loop, isLocal) {
        myPlaySound(sound, loop, position);
        if(!isLocal) {
            server.sendToServer({
                type : "sound",
                sound : sound,
    ...
    this.setListenerPosition = function(position) {
        myPosition = position;
    }
