...
var oppPos = env.getOpponentPosition();
...
if (aims.length == 1) {
    ...
    for (var i = 0; i < aims.length; i++) {
        var di = d(pos, aims[i]);
        var wi = d(oppPos, aims[i]) > di ? 0.5 : 1;
        for (var j = 0; j < aims.length; j++) {
            if (i != j) {
                var dj = d(aims[i], aims[j]);
                var wj = d(oppPos, aims[j]) > d(pos, aims[j]) ? 0.5 : 1;
                for (var k = 0; k < aims.length; k++) {
                    if (i != k && j != k) {
                        var dk = d(aims[j], aims[k]);
                        var wk = d(oppPos, aims[k]) > d(pos, aims[k]) ? 0.5 : 1;
                        var dd = di * wi + dj * wj + dk * wk;
                        if (min < 0 || dd < min) {
                            min = dd;
                            index = i;
                        }
